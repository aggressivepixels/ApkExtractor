...
import com.jonathan.apkextractor.BuildConfig;
import com.jonathan.apkextractor.observer.InstalledAppsObserver;
import com.jonathan.apkextractor.observer.SystemLocaleObserver;
...
public class AppListLoader extends AsyncTaskLoader<List<AppEntry>> {
    ...
    public AppListLoader(Context ctx, boolean showSystemApps) {
    ...
    public List<AppEntry> loadInBackground() {
        if (BuildConfig.DEBUG) Log.i(TAG, " loadInBackground() called! ");
        ...
        for (int i = 0; i < apps.size(); i++) {
            if ((apps.get(i).flags & ApplicationInfo.FLAG_SYSTEM) != 0 && !mShowSystemApps) {
                continue;
            }
            AppEntry entry = new AppEntry(this, apps.get(i));
            entry.loadLabel(getContext());
            entry.getIcon(); //It will load the icon, if any
            entries.add(entry);
        }
    ...
    public void deliverResult(List<AppEntry> apps) {
        ...
        if (oldApps != null && oldApps != apps) {
            if (BuildConfig.DEBUG) Log.i(TAG, "Releasing any old data associated with this Loader.");
    ...
    protected void onStartLoading() {
        ...
        if (mApps != null) {
            ...
            if (BuildConfig.DEBUG) Log.i(TAG, " Delivering previously loaded data to the client...");
            deliverResult(mApps);
        }
        ...
        if (takeContentChanged()) {
            ...
            if (BuildConfig.DEBUG) Log.i(TAG, " A content change has been detected... so force load! ");
            forceLoad();
        } else if (mApps == null) {
            // If the current data is null... then we should make it non-null! :)
            if (BuildConfig.DEBUG) Log.i(TAG, " The current data is data is null... so force load! ");
            forceLoad();
        }
    }
